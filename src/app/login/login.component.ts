...
export class LoginComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (encryptedUsername) {
      this.usernameOrEMail = CryptoJS.AES.decrypt(encryptedUsername, 'your-secret-key').toString(CryptoJS.enc.Utf8);
  ...
  async login(guestLogin: boolean = false, event: Event) {
    try {
      ...
      let response = null;
      if (!guestLogin) {
        response = await this.authService.loginWithUsernameOrEmail(this.usernameOrEMail, this.password);
      } else {
  ...
  loginProcess(response: any) {
    ...
    if (this.rememberMe) {
      const encryptedUsername = CryptoJS.AES.encrypt(this.usernameOrEMail, 'your-secret-key').toString();
